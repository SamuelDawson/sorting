...
static char *int_to_int_1() {
  int val = 0;
  void *void_val = &val;
  mu_assert("val != 5", int_to_int(void_val) == 0);
  return 0;
}
...
static char *int_to_int_2() {
  int val = INT_MAX;
  void *void_val = &val;
  mu_assert("val != INT_MAX", int_to_int(void_val) == INT_MAX);
  return 0;
}
...
static char *int_to_int_3() {
  int val = INT_MIN;
  void *void_val = &val;
  mu_assert("val != INT_MIN", int_to_int(void_val) == INT_MIN);
  return 0;
}
...
static char *swap_1() {
  ...
  int b = INT_MAX;
  swap(&a, &b, sizeof(a));
  mu_assert("Int swap failed", a == INT_MAX && b == INT_MIN);
  return 0;
}
...
static char *swap_3() {
  ...
  swap(&a, &b, sizeof(a));
  swap(&a, &b, sizeof(a));
  mu_assert("Swap twice failed", a == -1.1 && b == 100);
  return 0;
}
